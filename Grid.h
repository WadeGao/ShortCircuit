...
private:
	...
	//通过数据集获得线路原始导纳矩阵
	Eigen::MatrixXcf setYxFromSheet(const Eigen::MatrixXf& line_data_sheet, std::map<std::pair<size_t, size_t>, std::pair<cf, cf>>& socketData);
...
public:
	...
	Eigen::MatrixXcf getYx(const SEQUENCE whichSeq);
};
